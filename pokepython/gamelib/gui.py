...
from pokepython.gamelib.questionBank import questionBank

GAMELIB = Path(__file__).parent / ""
...
class applicationGUI():
    ...
    def __init__(self, master, level):
        ...
        # BACKGROUND
        self.bgImg = PhotoImage(file=str(GAMELIB / "img" / "bg.gif"))
        ...
        self.heartImg = PhotoImage(file=str(GAMELIB / "img" / "heart.gif"))
        ...
        self.player1Path = str(random.choice(list((GAMELIB / "players").iterdir())))
    ...
    def youWin(self, event):
        ...
        if self.level < 4:
            ...
            self.pikachuVictoryPath = str(GAMELIB / "img"/ "eagerpikachu.gif")
            ...
            self.pikachuVictoryPath = str(GAMELIB / "img"/ "victorypikachu.gif")
            self.pikachuVictoryObject = AnimatedGif(self.pikachuVictoryPath)
            self.pikachuVictory = self.canvas.create_image(650, 160, image=self.pikachuVictoryObject[2])
    ...
    def youLose(self, event):
        ...
        self.pikachuDancePath = str(GAMELIB / "img"/ "encouragingpikachu.gif")
...
def play():
    ...
    pg.mixer.init()
    pg.mixer.music.load(str(GAMELIB / 'audio' / 'Rather_Be_8_Bit_Remix_Version_[Tribute_to_Clean_Ba.wav'))
